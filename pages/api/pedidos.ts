...
const endpointPedidos = nc()
  ...
  .get(async (req: NextApiRequest, res: NextApiResponse) => {    
    try {
        ...
        const livroEncontrado = await LivrosModel.findById(pedidoEncontrado.idLivro);
        ...
        return res.status(500).json({ msg: "Ocorreu um erro ao buscar o pedido." });
    }
  })
  .delete(async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        if (pedidoEncontrado.status === "FINALIZADO") {
            return res.status(400).json({ msg: "O pedido foi finalizado e não pode ser removido"});
        }
        ...
        return res.status(500).json({ msg: "Ocorreu um erro ao deletar o pedido." });
    }
  })
  .put(async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        return res.status(500).json({ msg: "Ocorreu um erro ao atualizar o pedido." });
    }
  });
